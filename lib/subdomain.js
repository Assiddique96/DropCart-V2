...
const APP_URL = process.env.NEXT_PUBLIC_APP_URL || "https://shpinx.com";

/** The bare apex domain, e.g. "shpinx.com" (derived from NEXT_PUBLIC_APP_URL). */
...
/** Build an absolute https URL on a store subdomain for the given path. */
export function buildStoreSubdomainUrl(username, pathname = "/", search = "") {
  const cleanUsername = (username || "").trim().toLowerCase();
  if (!cleanUsername) return pathname || "/";

  const cleanPath = pathname && !pathname.startsWith("/") ? `/${pathname}` : pathname || "/";
  const cleanSearch = search && !search.startsWith("?") ? `?${search}` : search;
  return `https://${cleanUsername}.${ROOT_DOMAIN}${cleanPath}${cleanSearch || ""}`;
}
