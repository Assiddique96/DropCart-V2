...
const Navbar = () => {
  ...
  const storeCta = isSeller ? { label: "Store Dashboard", href: "/store", emoji: "🏪" } : { label: "Create a Store", href: "/create-store", emoji: "🏪" };
    ...
    <header className="sticky top-0 z-50 bg-white dark:bg-slate-950 shadow-2xl">
      ...
      <div className="hidden md:flex items-center justify-between px-6 lg:px-10 h-12 text-xs bg-slate-50/80 dark:bg-slate-900/60 backdrop-blur-xl border-b border-slate-200/50 dark:border-slate-700/50 shadow-[0_4px_20px_rgba(0,0,0,0.05)] dark:shadow-[0_4px_20px_rgba(0,0,0,0.3)]">
        <div className="flex items-center gap-6">
          <button type="button" className="inline-flex items-center gap-2 hover:text-cyan-500 dark:hover:text-cyan-400 transition-all duration-300 hover:scale-105 group">
      ...
      <div className="hidden md:block border-t border-slate-100 dark:border-slate-700/50 bg-slate-50/70 dark:bg-slate-900/60 backdrop-blur-xl shadow-[0_4px_20px_rgba(0,0,0,0.03)] dark:shadow-[0_4px_20px_rgba(0,0,0,0.2)]">
        <div className="max-w-7xl mx-auto px-4 lg:px-10 flex items-center gap-6 h-12 text-xs font-bold text-slate-600 dark:text-slate-300 overflow-x-auto no-scrollbar">
          <Link href="/shop?sort=promo" className="text-fuchsia-500 dark:text-fuchsia-400 font-bold hover:scale-110 transition-all animate-pulse">SALE</Link>
      ...
      {catalogOpen && (
        <div ref={catalogRef} className="hidden md:block absolute top-[140px] left-0 z-40">
          <div className="flex">
            <div className="w-64 bg-white dark:bg-slate-950 border-r border-slate-100 dark:border-slate-700/50 shadow-xl dark:shadow-[0_0_50px_rgba(34,211,238,0.2)] max-h-[calc(100vh-160px)] overflow-y-auto">
              {CATEGORIES.map((cat) => (
                <button
                  key={cat.name}
            ...
            <div className="w-[420px] bg-slate-50 dark:bg-slate-900 shadow-xl dark:shadow-[0_0_50px_rgba(232,121,249,0.2)] border border-l-0 border-slate-100 dark:border-slate-700/50 p-5 max-h-[calc(100vh-160px)] overflow-y-auto">
      ...
      {mobileOpen && (
        <div ref={mobileRef} className="md:hidden absolute top-full left-0 right-0 bg-white dark:bg-slate-950 shadow-xl border-t border-slate-100 dark:border-slate-700/50 shadow-[0_0_50px_rgba(34,211,238,0.1)] dark:shadow-[0_0_50px_rgba(34,211,238,0.2)] z-40 max-h-[85vh] overflow-y-auto">
          <div className="p-5 space-y-5">
            <form onSubmit={handleSearch} className="flex items-center gap-2 bg-slate-100 dark:bg-slate-900 px-4 py-3 rounded-full border border-slate-200 dark:border-slate-700/50 hover:border-cyan-400/50 shadow-[0_0_20px_rgba(34,211,238,0.08)]">
              <Search size={18} className="text-slate-400 dark:text-slate-500 shrink-0 animate-pulse" />
              <input className="w-full bg-transparent outline-none placeholder-slate-400 text-sm text-slate-700 dark:text-slate-200" type="text" placeholder="Search on Shpinx" value={search} onChange={(e) => setSearch(e.target.value)} required />
            </form>
            ...
            <div className="p-5 bg-slate-50 dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700/50 text-sm shadow-[0_0_30px_rgba(167,139,250,0.1)] dark:shadow-[0_0_30px_rgba(167,139,250,0.2)]">
              ...
              <Link
                href={storeCta.href}
                onClick={() => setMobileOpen(false)}
                className="inline-flex items-center gap-1 text-xs font-bold text-white bg-gradient-to-r from-cyan-600 to-fuchsia-600 px-5 py-2 rounded-full hover:from-cyan-500 hover:to-fuchsia-500 transition-all duration-300 hover:scale-105 hover:shadow-[0_0_20px_rgba(34,211,238,0.5)]"
              >
                {isSeller ? "Go to dashboard →" : "Open your store →"}
              </Link>
            </div>
