...
const Hero = () => {
  ...
  const isVerifiedStore = store?.verificationStatus === "verified";
  ...
  const defaults = useMemo(
    () => ({
      ...
      /** Quick filter links – all pointing to valid shop routes */
      quickFilters: [
        { label: "New arrivals", href: "/shop?sort=newest" },
        { label: "Top sellers", href: "/shop?sort=popular" },
        { label: "Electronics", href: "/shop?category=Electronics" },
        { label: "Computers", href: "/shop?category=Laptops" },
  ...
  const [fi, setFi, animating] = useInfiniteCarousel(featuredSlides.length, 6500);
  const [p1i, setP1i] = useFiniteCarousel(promo1Slides.length, 5500);
  const [p2i, setP2i] = useFiniteCarousel(promo2Slides.length, 5500);

  /** VERIFIED STORES – real data */
  const [verifiedStores, setVerifiedStores] = useState([]);
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        const res = await fetch("/api/verified-stores");
        const { stores } = await res.json();
        if (!cancelled) setVerifiedStores(stores || []);
      } catch {
        if (!cancelled) setVerifiedStores([]);
      }
    })();
    return () => { cancelled = true; };
  }, []);

  /** JOIN AS VERIFIED STORE – smart routing */
  ...
  /** Micro promos with vendor-center smart link */
  const microPromos = useMemo(() => {
    return defaults.microPromos.map((p) => {
      if (p.key === "vendor-center") {
        ...
        return { ...p, href: "/create-store" };
      }
      return p;
    });
  }, [defaults.microPromos, user, hasStore, isVerifiedStore]);
    ...
    <section className="mx-3 sm:mx-4 md:mx-6">
      <div className="mx-auto flex max-w-7xl flex-col gap-3 pt-3 text-xs sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-wrap items-center gap-2 text-slate-700 dark:text-slate-200">
          <span className="rounded-full bg-slate-900 dark:bg-slate-800 px-3 py-1 text-[11px] font-semibold uppercase tracking-wide text-white border border-slate-700 dark:border-slate-600">
            Shpinx Marketplace
          </span>
      ...
      <VerifiedStoresSection
        stores={
          verifiedStores.length
            ? verifiedStores
            : [
                { id: 1, name: "Lagos Tech Hub", rating: 4.9, orders: "2.1k+", tag: "Enterprise store", href: "/shop" },
                { id: 2, name: "Abuja Gadgets Pro", rating: 4.8, orders: "1.6k+", tag: "Verified SME", href: "/shop" },
                { id: 3, name: "Malta Electronics Lab", rating: 4.7, orders: "980+", tag: "EU warehouse", href: "/shop" },
                { id: 4, name: "Krasnodar Digital Store", rating: 4.9, orders: "1.3k+", tag: "Priority store", href: "/shop" },
              ]
        }
      ...
      <div className="mx-auto mt-4 grid max-w-7xl grid-cols-2 gap-2 text-[11px] sm:grid-cols-4 sm:text-xs">
        {microPromos.map((p) => (
          <Link
            key={p.key}
            href={p.href}
            className="group flex items-center justify-between gap-2 rounded-2xl bg-white dark:bg-slate-800 px-3 py-2 text-slate-800 dark:text-slate-200 transition hover:bg-slate-50 dark:hover:bg-slate-700 hover:-translate-y-0.5 border border-slate-200 dark:border-slate-600 dark:hover:border-slate-500"
          >
            ...
            <ArrowRightIcon
              size={16}
              className="shrink-0 text-slate-400 transition-transform group-hover:translate-x-0.5 group-hover:text-slate-600 dark:group-hover:text-slate-200"
            />
          </Link>
        ))}
      </div>
      ...
      >
        ...
        <div className="absolute inset-0 z-10 flex flex-col justify-between p-4 sm:p-5">
          <div className="flex flex-col gap-1">
            <p className="max-w-[14rem] text-base font-semibold leading-tight text-white sm:text-lg drop-shadow-sm">
              {currentSlide.title || "Offers"}
            </p>
            <p className="text-[11px] text-slate-100 sm:text-xs drop-shadow-sm">
      ...
      {hasMany && (
        <div className="pointer-events-none absolute inset-y-0 left-0 right-0 flex items-center justify-between px-1">
          <button
            ...
            className="pointer-events-auto rounded-full bg-black/65 p-1 text-white hover:bg-black/80 border border-slate-500"
          >
...
function VerifiedStoresSection({ stores, onJoinVerifiedStore }) {
  ...
  return (
    <div className="mx-auto mt-6 max-w-7xl rounded-3xl bg-slate-50 dark:bg-slate-900 px-3 py-3 text-[11px] ring-1 ring-slate-200 dark:ring-slate-600 sm:px-4 sm:py-4 sm:text-xs">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <span className="inline-flex h-7 w-7 items-center justify-center rounded-full bg-slate-800 text-white">
        ...
        <button
          ...
          className="inline-flex items-center gap-1 rounded-full border border-slate-200 dark:border-slate-600 px-3 py-1 text-[10px] font-semibold text-slate-800 dark:text-slate-200 transition hover:border-slate-900 dark:hover:border-slate-400 hover:bg-slate-900 dark:hover:bg-slate-700 hover:text-white"
        >
