...
const Footer = () => {
  ...
  return (
    <footer className="mx-4 sm:mx-6 bg-white dark:bg-slate-950 border-t border-slate-100 dark:border-slate-800">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row items-start justify-between gap-10 py-10 border-b border-slate-200/60 dark:border-slate-800 text-slate-500 dark:text-slate-300">
          <div>
            <Link
              href="/"
              className="text-4xl font-semibold text-slate-600 dark:text-slate-100"
            >
              <span className="text-gray-600 dark:text-slate-200">
                Shp
              </span>
              inx
              <span className="text-gray-600 text-5xl leading-0" />
            </Link>
            <p className="max-w-[410px] mt-6 text-sm text-slate-500 dark:text-slate-400">
              Welcome to shpinx, your ultimate destination for the latest and
            ...
            <div className="flex items-center gap-3 mt-5">
              {socialIcons.map((item, i) => (
                <Link
                  href={item.link}
                  key={i}
                  className="flex items-center justify-center w-10 h-10 bg-slate-100 dark:bg-slate-800 hover:scale-105 hover:border border-slate-300 dark:border-slate-600 transition rounded-full"
                >
                  <item.icon />
                </Link>
              ))}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:flex md:flex-wrap justify-between w-full md:w-[45%] gap-5 text-sm">
            {linkSections.map((section, index) => (
              <div key={index}>
                <h3 className="font-medium text-slate-700 dark:text-slate-100 md:mb-5 mb-3">
                  {section.title}
                </h3>
                <ul className="space-y-2.5">
                  {section.links.map((link, i) => (
                    <li
                      key={i}
                      className="flex items-center gap-2 text-slate-500 dark:text-slate-300"
                    >
                      {link.icon && <link.icon />}
                      <Link
                        href={link.path}
        ...
        <p className="py-4 text-sm text-slate-500 dark:text-slate-500">
          Copyright 2026 © Shpinx All Right Reserved.
        </p>
      </div>
    </footer>
  );
