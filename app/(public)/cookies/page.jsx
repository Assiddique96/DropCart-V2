...
const DEFAULT_COOKIES_CONTENT = (
    <>
        ...
        <section>
            ...
            <p>Data supports user registration, KYC for store owners, facial verification, and location-based services. Lawful bases include explicit consent and necessity for service provision per NDPA. Cookies store session data securely, expiring after use or as specified.</p>
        </section>

        <section>
            <h2 className="text-slate-800 font-semibold mb-2">Managing cookies</h2>
            <p>Users can manage permissions via device settings or account preferences. Withdraw consent anytime without affecting prior processing; we honor NDPA rights to access, rectify, or erase data.</p>
        </section>
...
export default async function CookiesPolicyPage() {
    const { cookiesPolicy } = await getSiteContent()
    const cookiesContent = cookiesPolicy ? (
        <div className="whitespace-pre-line text-sm text-slate-600 leading-relaxed">{cookiesPolicy}</div>
    ) : (
        <div className="mt-10 space-y-6 text-sm text-slate-600 leading-relaxed">{DEFAULT_COOKIES_CONTENT}</div>
    )
