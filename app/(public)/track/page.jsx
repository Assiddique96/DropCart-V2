...
function TrackContent() {
  ...
  const lookup = async (t) => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/track?trackingNumber=${encodeURIComponent(tn)}`,
      );
      const data = await res.json();

      if (!res.ok) {
        throw new Error(
          data?.error || "Unable to find that tracking number.",
        );
      }
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-12 bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100">
      ...
      >
        <input
          ...
          maxLength={32}
          className="flex-1 px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-200 dark:focus:ring-indigo-500/40"
        />
        <button
          type="submit"
          disabled={loading}
      ...
      {/* Error State */}
      {error && (
        <div className="mt-5 p-4 rounded-xl border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-300">
          {error}
        </div>
      )}
      ...
      {order && (
        <div className="mt-6 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-sm">
          ...
          <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-900/70 border border-slate-100 dark:border-slate-700">
              <p className="text-xs uppercase tracking-widest text-slate-400 dark:text-slate-500">
                Store
              </p>
          ...
          <div className="mt-6">
            ...
            <div className="space-y-3">
              {(order.orderItems || []).map((it, idx) => (
                <div
                  key={idx}
                  className="flex items-center gap-3 p-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900/70"
                >
                  <div className="w-12 h-12 rounded-lg bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 overflow-hidden shrink-0">
                    {it.product?.images?.[0] ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center bg-slate-200 dark:bg-slate-700 text-[10px] text-slate-400 dark:text-slate-300">
                        No Image
                      </div>
                    )}
                  </div>
...
export default function TrackPage() {
  return (
    <Suspense
      fallback={
        <div className="max-w-3xl mx-auto px-6 py-20 text-center text-slate-500 dark:text-slate-400 bg-white dark:bg-slate-950">
          Loading tracker...
        </div>
      }
    >
