    ...
    return (
        <div className="text-slate-500 dark:text-slate-300 mb-40">
            ...
            <form onSubmit={e => toast.promise(handleAddCoupon(e), { loading: "Adding coupon..." })} className="max-w-lg text-sm">
                <h2 className="text-2xl mb-4">Add <span className="text-slate-800 dark:text-slate-100 font-medium">Coupon</span></h2>
                ...
                <div className="grid grid-cols-2 gap-3 mb-3">
                    <div>
                        ...
                        <input type="text" placeholder="e.g. SAVE20" required
                            className="w-full p-2.5 border border-slate-200 dark:border-slate-700 outline-slate-400 rounded-lg text-sm uppercase"
                                ...
                                return (
                                    <tr key={coupon.code} className={`hover:bg-slate-50 ${isExpired || isExhausted ? 'opacity-60' : ''}`}>
                                        ...
                                        <td className="py-3 px-4 hidden md:table-cell">
                                            <span className={`text-xs ${isExpired ? 'text-red-400' : 'text-slate-500 dark:text-slate-300'}`}>
            ...
            {confirmDelete && (
                <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50">
                    <div className="bg-white dark:bg-slate-900 rounded-xl shadow-lg p-6 max-w-sm w-full mx-4">
                        <h3 className="font-semibold text-slate-800 dark:text-slate-100 mb-2">Delete coupon <span className="font-mono">{confirmDelete}</span>?</h3>
                        <p className="text-sm text-slate-500 dark:text-slate-300 mb-4">This cannot be undone. Active users with this code will no longer be able to use it.</p>
