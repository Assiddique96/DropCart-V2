...
export default function SellerPayouts() {
    ...
    const cards = [
        ...
        { label: "Pending Balance", value: `${currency}${data.pendingBalance.toLocaleString()}`, icon: ClockIcon, color: data.pendingBalance > 0 ? "text-amber-600 bg-amber-50" : "text-slate-400 bg-slate-50" },
    ]

    return (
        <div className="text-slate-500 mb-28">
            <h1 className="text-2xl mb-1">Payout <span className="text-slate-800 font-medium">History</span></h1>
            <p className="text-xs text-slate-400 mb-6">Revenue is counted from delivered, paid orders only.</p>

            <div className="flex flex-wrap gap-4 mb-8">
                {cards.map((card, i) => (
                    <div key={i} className="flex items-center gap-4 border border-slate-200 rounded-lg p-4 px-6">
            ...
            {data.payouts.length === 0 ? (
                ...
                <div className="overflow-x-auto rounded-lg border border-slate-200 shadow-sm">
                    <table className="w-full text-sm text-left">
                        <thead className="bg-slate-50 text-gray-700 text-xs uppercase tracking-wider">
                            <tr>
                        ...
                        <tbody className="divide-y divide-slate-100 text-slate-700">
                            {data.payouts.map((payout) => (
                                <tr key={payout.id} className="hover:bg-slate-50 transition-colors">
                                    ...
                                    <td className="px-4 py-3 font-semibold text-slate-800">{currency}{payout.amount.toLocaleString()}</td>
