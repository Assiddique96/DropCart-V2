    ...
    return (
        <div className="text-slate-500 dark:text-slate-300 mb-28">
            <div className="flex items-center justify-between mb-6 flex-wrap gap-2">
                <h1 className="text-2xl">My <span className="text-slate-800 dark:text-slate-100 font-medium">Stores</span></h1>
            ...
            {stores.length === 0 ? (
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {stores.map((store) => (
                        <div key={store.id} className="border border-slate-200 dark:border-slate-700 rounded-xl p-4 bg-white dark:bg-slate-900">
                            <div className="flex items-center gap-3">
                                <Image src={store.logo} alt="" width={44} height={44} className="w-11 h-11 rounded-full object-cover border border-slate-100 dark:border-slate-800" />
                                <div className="min-w-0">
                                    <p className="font-medium text-slate-800 dark:text-slate-100 truncate">{store.name}</p>
                            ...
                            <div className="mt-3 flex items-center gap-2 text-xs">
                                ...
                                <span className={`px-2 py-0.5 rounded-full ${store.isActive ? "bg-blue-50 text-blue-700" : "bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-300"}`}>
