                    ...
                    {orders.map((order, index) => (
                        <div key={order.id} className="border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden shadow-sm">
                            ...
                            {selectedOrder?.id === order.id && (
                                <div className="px-4 py-4 text-sm text-slate-600 dark:text-slate-300 border-t border-slate-100 dark:border-slate-800 space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <div className="space-y-2">
                                            {order.orderItems.map((item, i) => (
                                                <div key={i} className="flex items-center gap-3 border border-slate-100 dark:border-slate-800 rounded-lg p-2">
                                                    <img src={item.product?.images?.[0]} alt="" className="w-12 h-12 object-cover rounded" />
                                                    <div className="flex-1">
                                                        <p className="font-medium text-slate-700 dark:text-slate-200">{item.product?.name}</p>
                                                    ...
                                                    <p className="font-semibold text-slate-800 dark:text-slate-100 text-sm">{currency}{(item.price * item.quantity).toLocaleString()}</p>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                                        ...
                                        <div className="flex flex-wrap gap-3 pt-2 border-t border-slate-100 dark:border-slate-800">
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                <select value={order.status}
                                                    ...
                                                    className="border border-slate-200 dark:border-slate-700 rounded text-xs p-1.5 outline-none">
                                            ...
                                            <button
                                                ...
                                                className="flex items-center gap-1 text-xs px-2.5 py-1.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded hover:bg-slate-200 transition">
                                    ...
                                    {fulfillmentOpen === order.id && (
                                        <div className="border border-slate-100 dark:border-slate-800 rounded-lg p-3 bg-slate-50 dark:bg-slate-900">
                                            <p className="text-xs font-semibold text-slate-500 dark:text-slate-300 mb-3">Mark Fulfilled Quantities</p>
                                            <div className="space-y-2">
                                                {order.orderItems.map(item => (
                                                    <div key={item.productId} className="flex items-center gap-3">
                                                        <span className="text-xs flex-1 text-slate-700 dark:text-slate-200 truncate">{item.product?.name}</span>
                                                        ...
                                                        <input
                                                            ...
                                                            className="border border-slate-200 dark:border-slate-700 rounded p-1 text-xs w-16 outline-none text-center"
                                                        />
                                                    </div>
                                                ))}
                                            </div>
            ...
            {cancelModal && (
                <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50">
                    <div className="bg-white dark:bg-slate-900 rounded-xl shadow-lg p-6 max-w-md w-full mx-4">
                        <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100 mb-1">Cancel Order?</h3>
                        <p className="text-sm text-slate-500 dark:text-slate-300 mb-3">Shipped orders cannot be cancelled.</p>
